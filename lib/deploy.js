...
import ora from 'ora'
import { getCurrentDate, successLog, failLog } from '../tools/index.js'
...
async function removeLocalDist(envConfig) {
    ...
    const isExist = existsSync(local.distPathFull)
    if (!isExist) {
        return Promise.resolve()
    }
    return new Promise((resolve, reject) => {
        rm(local.distPathFull, { recursive: true }, (err) => {
            if (err) {
                reject(false)
...
async function removeLocalZip(envConfig) {
    ...
    const isExist = existsSync(local.zipPathFull)
    if (!isExist) {
        return Promise.resolve()
    }

    return new Promise((resolve, reject) => {
        rm(local.zipPathFull, (err) => {
            if (err) {
                reject(false)
                failLog('本地zip包删除失败')
                throw err
